...
library(igraph)
g1 <- graph_from_edgelist(as.matrix(cbind("CLOPIDOGREL", statclop[,2])), directed=T ) 
...
plot(g1, edge.color="orange", layout=l1)
...
clop2$anc1 <- res1 
clop2$anc2 <- res2 
statclop1 = clop2[, .N, by=list(anc1,root)]
...
g2 <- graph_from_edgelist(as.matrix(cbind("CLOPIDOGREL", statclop1[,2])), directed=T ) 
statclop1$N[statclop1$N == 1 ] <- 0
E(g2)$weight <- statclop1$N         
E(g2)$width <- 1+E(g2)$weight*2
V(g2)$size <- 20
...
# ancestor II
g3 <- graph_from_edgelist(as.matrix(cbind("CLOPIDOGREL", statclop2[,2])), directed=T ) 
statclop2$N[statclop2$N == 1 ] <- 0
E(g3)$weight <- statclop2$N         
E(g3)$width <- 1+E(g3)$weight*2
V(g3)$size <- 20
V(g3)$vertex.label.cex <-0.5
V(g3)$frame.color <- "white"
V(g3)$color <-  c("lightgrey", statclop2$mypalette)
l3 <- layout_as_star(g3)
#plot(g3, vertex.label.cex= 0.8, layout=l)

par(mfrow=c(2,2), mar=c(1,1,1,1)) 
plot(g1, vertex.label.cex= 0.8, layout=l1*0.6, main="Drug")
plot(g2, vertex.label.cex= 0.8, layout=l2*0.6, main="Ancestor I")
plot(g3, vertex.label.cex= 0.8, layout=l3*0.6, main="Ancestor II")
dev.off()
```
